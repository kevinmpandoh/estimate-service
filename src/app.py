    ...
    try: 
        ...
        for key in ['MEREK', 'TIPE UNIT', 'KERUSAKAN']:
            ...
            if input_data[key] in le.classes_:
                ...
                return jsonify({"success": False, "message": f"{key} '{input_data[key]}' tidak dikenal."}), 404
...
def train_model():
    ...
    try:
        ...
        df = df[df['KERUSAKAN'].isin(kerusakan_counts[kerusakan_counts >= 9].index)]
        ...
        df["KERUSAKAN"] = df["KERUSAKAN"].replace(damage_map).str.lower()
        ...
        encoders = {}
        for col in ['MEREK', 'TIPE UNIT', 'KERUSAKAN', 'ESTIMASI_WAKTU', 'KATEGORI_BIAYA']:
            le = LabelEncoder()
        ...
        # kategori_map = {
        #     'Rendah': 'Rendah (0 - 200.000)',
        #     'Sedang': 'Sedang (200.001 - 500.000)',
        #     'Tinggi': 'Tinggi (> 500.000)'
        # }

        # Decode kolom menggunakan LabelEncoder masing-masing
        df_output['MEREK'] = encoders['MEREK'].inverse_transform(df['MEREK'])
        df_output['TIPE UNIT'] = encoders['TIPE UNIT'].inverse_transform(df['TIPE UNIT'])
        df_output['KERUSAKAN'] = encoders['KERUSAKAN'].inverse_transform(df['KERUSAKAN'])
        df_output['ESTIMASI_WAKTU'] = encoders['ESTIMASI_WAKTU'].inverse_transform(df['ESTIMASI_WAKTU'])
        df_output['KATEGORI_BIAYA'] = encoders['KATEGORI_BIAYA'].inverse_transform(df['KATEGORI_BIAYA'])
        ...
        df_output['KATEGORI_BIAYA'] = df_output['KATEGORI_BIAYA'].map(kategori_map)
        ...
        cm_labels = encoders['KATEGORI_BIAYA'].classes_.tolist()
        cm_list = cm.tolist()
